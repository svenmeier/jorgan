...
JNIEXPORT
jobject Java_jorgan_fluidsynth_Fluidsynth_init(JNIEnv* env, jclass jclass, jstring jname, jint jchannels, jint jpolyphony, jfloat jsampleRate, jstring jaudioDriver, jstring jaudioDevice, jint jbuffers, jint jbufferSize) {
...
jobject JNICALL Java_jorgan_fluidsynth_Fluidsynth_getAudioDevices(JNIEnv* env, jclass jclass, jstring jaudioDriver) {
  ...
  char *key = (char *)calloc(strlen(prefix) + strlen(audioDriver) + strlen(suffix) + 1, 
                        sizeof(char));
  strcat(key, prefix);
