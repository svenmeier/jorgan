...
public class OrganShell implements UI {
  ...
  private static ResourceBundle resources = ResourceBundle.getBundle("jorgan.shell.resources");
  ...
  protected void showElementStatus(Element element) {
    ...
    if (problems != null) {
      for (int p = 0; p < problems.size(); p++) {
        ...
        showMessage(pattern, new Object[]{element.getName(), problem.getValue()});
      }
    }
  }
   ...
   * Show a message.
   *
   * @param pattern   pattern of message to show
    ...
    public void execute(String param) {
      ...
      if (param == null) {
        ...
        for (int r = 0; r < recents.size(); r++) {
          ...
          showMessage("command.recent.list", new Object[]{new Integer(r + 1), recent});
        }
      } else {
        try {
          ...
          showMessage("command.recent.format", new Object[]{from, to});
        }
      }
    }
  }
