...
public class ControllerPanel extends JPanel {
	...
	private void read() {
		this.deviceComboBox.setModel(ComboBoxUtils
				.createModelWithNull(DevicePool.instance().getMidiDeviceNames(
						Direction.IN)));
		this.deviceComboBox.setSelectedItem(controller.getInput());

		switchRows = new ArrayList<SwitchRow>();
		for (Switch aSwitch : controller.getReferenced(Switch.class)) {
			switchRows.add(new SwitchRow(aSwitch));
		}

		continuousRows = new ArrayList<ContinuousRow>();
		for (Continuous aContinuous : controller
	...
	public void apply() {
		controller.setInput(getDeviceName());
	...
	private abstract class RecordAction extends BaseAction {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				int result = messageBox.show(ControllerPanel.this);
